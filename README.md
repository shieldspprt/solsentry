# SolSentry: Solana AI Agent Risk Engine

SolSentry is an open source quantitative risk engine, position liquidation monitor, and guardrail policy middleware for Solana AI trading agents.
...
AI trading agents (ElizaOS, Solana Agent Kit, Claude, GPT, custom bots) query SolSentry via Model Context Protocol (MCP) or REST APIs before performing swaps, loans, perp trades, or liquidity provision:
...
Human fund managers and delegators use the SolSentry web interface to monitor and protect trading portfolios:
...
SolSentry provides standard Model Context Protocol (MCP) tools (responses include structured content):

- solsentry_preflight — one-shot PROCEED / DO_NOT_PROCEED verdict (risk + policy combined), with `maxAllowedUsd`, drivers, trend, and alternatives. **Call this first.**
- solsentry_check_protocol_risk — provenance-tagged safety score + confidence band, factor breakdown, drivers, what-would-flip, trend
- solsentry_evaluate_policy — portfolio-aware guardrail check returning `maxAllowedUsd` and safer alternatives
- solsentry_stress_test — price-shock simulation: liquidations, capital at risk, cascade, time-to-liquidation, collateral to restore safety
- solsentry_get_position_health — pass `walletAddress` to read **real on-chain positions with live health factors** (Kamino live; Drift pending). Also over REST: `POST /api/v1/positions/read`
- solsentry_get_protocol_list
- solsentry_get_business_ratios
...
MIT License. Copyright (c) 2026 SolSentry Team.
