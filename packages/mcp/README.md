...
It is a thin stdio proxy. It holds no keys and runs no engine code. The transport runs locally so your client can add it in one line, while the risk engine, live data grounding, and API keys stay server side. The package exposes the `solsentry-mcp` binary name.
